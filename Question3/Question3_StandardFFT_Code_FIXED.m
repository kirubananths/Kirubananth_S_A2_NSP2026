...
% Uses STANDARD FFT METHOD (not multi-taper)
%
...
% - Coherence: |E[S12]| / sqrt(E[S11] * E[S22])
...
%% ========================================================================
%  COMPUTE CROSS AND AUTO SPECTRA USING STANDARD FFT
%  ========================================================================

fprintf('\nComputing cross and auto spectra using standard FFT method...\n');

% FFT parameters
N = size(dataPO3, 1);  % Number of samples
nfft = 2^nextpow2(N);   % Next power of 2 for efficiency
fpass = [0 50];         % Frequency range: 0-50 Hz
...
% Preallocate
FFT_PO3 = zeros(nfft, numTrials);
FFT_AF4 = zeros(nfft, numTrials);

for tr = 1:numTrials
    % Compute FFT for each trial
    FFT_PO3(:, tr) = fft(dataPO3(:, tr), nfft);
    FFT_AF4(:, tr) = fft(dataAF4(:, tr), nfft);
end

% Keep only positive frequencies (0 to Nyquist)
FFT_PO3 = FFT_PO3(1:nfft/2+1, :);
FFT_AF4 = FFT_AF4(1:nfft/2+1, :);

% Extract frequencies in fpass range
FFT_PO3 = FFT_PO3(freqIdx, :);  % freq × trials
FFT_AF4 = FFT_AF4(freqIdx, :);  % freq × trials
...
% Cross-spectrum: S12(f,trial) = FFT_PO3(f,trial) * conj(FFT_AF4(f,trial))
% Auto-spectra: S11(f,trial) = |FFT_PO3(f,trial)|^2
%               S22(f,trial) = |FFT_AF4(f,trial)|^2

S12 = conj(FFT_PO3) .* FFT_AF4;  % freq × trials (cross-spectrum)
S11 = conj(FFT_PO3) .* FFT_PO3;  % freq × trials (PO3 power spectrum)
S22 = conj(FFT_AF4) .* FFT_AF4;  % freq × trials (AF4 power spectrum)

% Convert to real for auto-spectra (should already be real, but make explicit)
S11 = real(S11);
S22 = real(S22);
...
legend([h1, h2, h3], ...
    ...
    sprintf('WPLI (mean=%.3f, range=[%.3f, %.3f])', ...
        mean(wpli, 'omitnan'), min(wpli), max(wpli)), ...
...
stats_text = sprintf(['Number of trials: %d\n' ...
                      ...
                      'Method: Standard FFT'], ...
                     N_trials, f(1), f(end));
annotation('textbox', [0.15, 0.72, 0.3, 0.18], ...
    'String', stats_text, ...
    'FontSize', 10, ...
...
title('Coherence: PO3-AF4 (M1, Unipolar, Standard FFT)', 'FontSize', 16, 'FontWeight', 'bold');
grid on;
xlim([fpass(1) fpass(2)]);
...
title('WPLI: PO3-AF4 (M1, Unipolar, Standard FFT)', 'FontSize', 16, 'FontWeight', 'bold');
grid on;
xlim([fpass(1) fpass(2)]);
...
title('PPC: PO3-AF4 (M1, Unipolar, Standard FFT)', 'FontSize', 16, 'FontWeight', 'bold');
grid on;
xlim([fpass(1) fpass(2)]);
...
fprintf('  Standard FFT (not multi-taper)\n');
fprintf('  FFT length: %d points\n', nfft);
fprintf('  No frequency smoothing (unlike multi-taper with W=±4Hz)\n');
fprintf('  Higher frequency resolution but more spectral variance\n');
