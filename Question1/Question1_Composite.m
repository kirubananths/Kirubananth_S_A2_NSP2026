...
% Parts: 1(a), 1(b), 1(c)
% MODIFIED: Creates composite figures with subplots while keeping individual plot appearance identical
...
% Collect phase differences for all pairs
allPhaseDiffs_1a = cell(1, 3);
allTitles_1a = cell(1, 3);
...
for pairIdx = 1:size(electrodePairs, 1)
    ...
    % Store data
    allPhaseDiffs_1a{pairIdx} = phaseDiffs;
    allTitles_1a{pairIdx} = sprintf('Phase Difference: %s - %s at %.0f Hz', elec1Name, elec2Name, targetFreq_1a);
end

% Create composite figure for Q1(a)
fprintf('\nCreating composite figure for Question 1(a)...\n');
createCompositeFigure(allPhaseDiffs_1a, allTitles_1a, 1, 3, outputDir, '1a_composite');
...
allGroupPhaseDiffs_1b = cell(1, 6);
allGroupTitles_1b = cell(1, 6);
...
for groupIdx = 1:6
    ...
    allGroupPhaseDiffs_1b{groupIdx} = groupPhaseDiffs;
    allGroupTitles_1b{groupIdx} = sprintf('Group %d: %s\n%s at %.0f Hz', ...
        groupIdx, groupNameList{groupIdx}, seedElectrode, targetFreq_1a);
end

% Create composite figure for Q1(b)
fprintf('\nCreating composite figure for Question 1(b)...\n');
createCompositeFigure(allGroupPhaseDiffs_1b, allGroupTitles_1b, 2, 3, outputDir, '1b_composite');
...
allGroupPhaseDiffs_1c = cell(1, 6);
allGroupTitles_1c = cell(1, 6);
...
for groupIdx = 1:6
    ...
    % Store for this group
    allGroupPhaseDiffs_1c{groupIdx} = groupPhaseDiffs_multi;
    allGroupTitles_1c{groupIdx} = sprintf('Group %d: %s\nSeeds: %s, Freq: %.0f-%.0f Hz', ...
        groupIdx, groupNameList_multi{groupIdx}, ...
        strjoin(seedElectrodes, ', '), freqRange(1), freqRange(2));
end

% Create composite figure for Q1(c)
fprintf('\nCreating composite figure for Question 1(c)...\n');
createCompositeFigure(allGroupPhaseDiffs_1c, allGroupTitles_1c, 2, 3, outputDir, '1c_composite');
...
fprintf('  Question 1(a): 1a_composite (3 electrode pairs in 1×3 layout)\n');
fprintf('  Question 1(b): 1b_composite (6 electrode groups in 2×3 layout)\n');
fprintf('  Question 1(c): 1c_composite (6 multi-seed groups in 2×3 layout)\n');
fprintf('Total plots: 3 composite figures (each in .png and .fig format)\n');
...
function createCompositeFigure(allPhaseDiffs, allTitles, nRows, nCols, outputDir, savePrefix)
    % Create a composite figure with multiple rose plots in subplot layout
    % Keeps the EXACT same appearance as individual plots
    
    nPlots = length(allPhaseDiffs);
    ...
    for plotIdx = 1:nPlots
        ...
        % Plot rose plot in current axes (EXACT same code as original function)
        phaseDiffs = allPhaseDiffs{plotIdx};
        titleStr = allTitles{plotIdx};
        ...
        % Calculate position in figure normalized coordinates for this subplot
        subplotPos = get(gca, 'Position');  % [left bottom width height] in figure coords
        
        % Position textbox relative to subplot (bottom-left area)
        dim = [subplotPos(1) + 0.02, subplotPos(2) + 0.02, 0.15, 0.08];
